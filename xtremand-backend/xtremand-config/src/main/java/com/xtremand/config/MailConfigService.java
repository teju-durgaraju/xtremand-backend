...
public class MailConfigService {
    ...
    public MailConfig saveOrUpdateMailConfig(MailConfigInput input, Authentication authentication) {
        User user = userRepository.fetchByUsername(authentication.getName());

        SmtpDetails details = validateMailConfigInput(input);
        Optional<MailConfig> optionalConfig = mailConfigRepository.findByEmail(input.getEmail());
        MailConfig config;

        if (optionalConfig.isPresent()) {
            config = optionalConfig.get();
            config.setConfigType(EmailConfigType.valueOf(input.getConfigType()));
    ...
    private SmtpDetails validateMailConfigInput(MailConfigInput input) {
        if (input.getConfigType() == null || !isValidConfigType(input.getConfigType())) {
            throw new IllegalArgumentException("Invalid config type: " + input.getConfigType());
        }
        EmailConfigType configType = EmailConfigType.valueOf(input.getConfigType());
    ...
    public Optional<MailConfigDTO> findAllMailConfigsByUser(Authentication authentication) {
        User user = userRepository.fetchByUsername(authentication.getName());
    ...
    public MailConfig updateMailConfig(Long id, MailConfigInput input, Authentication authentication) {
        ...
        SmtpDetails details = validateMailConfigInput(input);
        existingConfig.setConfigType(EmailConfigType.valueOf(input.getConfigType()));
