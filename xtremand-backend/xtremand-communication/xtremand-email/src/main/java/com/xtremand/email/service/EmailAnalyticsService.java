...
public class EmailAnalyticsService {
    ...
    public EmailAnalyticsService(EmailAnalyticsRepository repo, ContactService contactService,
                                 ContactListEmailAnalyticsRepository mappingRepo) {
        this.repo = repo;
    ...
    private User getAuthenticatedUser(Authentication authentication) {
        // Adjust for your security principal type. Assuming principal is User entity.
    ...
    public List<SentEmailWithReplyFlagDto> getSentEmailsWithReplyStatusForUser(Long id, Authentication authentication) {
        User user = getAuthenticatedUser(authentication);
        MailConfig config = mailConfigRepository.findByCreatedBy(user)
        ...
        try {
            emailInboxPollService.pollAndStoreInbox(fromEmail, passwordOrToken, useOAuth);
        } catch (Exception e) {
            e.printStackTrace();
        }
